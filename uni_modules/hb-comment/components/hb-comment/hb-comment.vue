<template>
	<view class="hb-comment">
					...
					<view class="comment-main" :id="generateCommentID(item.id)">
						...
						<view class="comment-main-top">
							...
							<view class="zan-box" @click="like(item)">
								<span :class="item.hasLike ? 'isLike' : 'notLike'">{{ item.likeNum == 0 ? '抢首赞' :
		item.likeNum
}}</span>
						...
						<view class="comment-sub-box">
							<view class="comment-sub-item" v-for="each in item.children" :key="each.id">
								...
								<view class="comment-main" :id="generateCommentID(each.id)">
									<view class="sub-comment-main-top">
										<view class="nick-name">{{ each.nickName }}</view>
										<view class="zan-box" @click="like(each)">
											<span :class="each.hasLike ? 'isLike' : 'notLike'">{{ each.likeNum == 0 ?
		'抢首赞' : each.likeNum
}}</span>
		...
		<!-- 无评论-end -->
		<!-- 新增评论-start -->
		<view id="last-comment" class="comment-submit-box" v-if="submit" @click="closeInput">
...
export default {
	...
	methods: {
		getBackgroundCSS(commendID) {
			console.debug("current comment ID: " + commendID + ", scrollToCommentID: " + this.commentData.scrollToCommentID)
			if (commendID == this.commentData.scrollToCommentID) {
				return "hightlight-comment"
			}
			return "dummy-css"
		},

		generateCommentID(id) {
			let cid = this.COMMENT_ID_PREFIX + id
		...
		scrollToComment(commentID) {
			let cid = this.generateCommentID(commentID)
			this.$nextTick(() => {
				this.setPageScrollTo("#" + cid);
				console.debug("scrolled to comment ", cid)
			})
		},
